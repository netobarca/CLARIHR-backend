...
public sealed class EmailVerificationToken : AuditableEntity
{
    ...
    private EmailVerificationToken(long userId, string tokenHash, DateTime expirationUtc)
    {
        if (userId <= 0)
        {
    ...
    public static EmailVerificationToken Issue(long userId, string tokenHash, DateTime expirationUtc) =>
