...
internal abstract class PersonnelFileEmployeeCommandHandlerBase
{
    ...
    /// <summary>Manage gate for one-time deductions (REQ-009): the dedicated
    /// <c>PersonnelFiles.ManageOneTimeDeductions</c> permission (or Admin / IAM super-admin). HR-only.</summary>
    protected static async Task<(Result<TResponse>? Failure, PersonnelFile? File)> LoadForManageOneTimeDeductionsAsync<TResponse>(
        Guid personnelFileId,
        Guid concurrencyToken,
        ITenantContext tenantContext,
        IPersonnelFileAuthorizationService authorizationService,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var authorizationResult = await authorizationService.EnsureCanManageOneTimeDeductionsAsync(tenantContext.TenantId.Value, cancellationToken);
        if (authorizationResult.IsFailure)
        {
            return (Result<TResponse>.Failure(authorizationResult.Error), null);
        }
    ...
    /// deliberately EXCLUDED. The TRIPLE anti-self checks are enforced in the handler.</summary>
    protected static async Task<(Result<TResponse>? Failure, PersonnelFile? File)> LoadForAuthorizeOneTimeDeductionsAsync<TResponse>(
        Guid personnelFileId,
        ITenantContext tenantContext,
        IPersonnelFileAuthorizationService authorizationService,
        IPersonnelFileRepository personnelFileRepository,
        CancellationToken cancellationToken)
    {
        ...
        var authorizationResult = await authorizationService.EnsureCanAuthorizeOneTimeDeductionsAsync(tenantContext.TenantId.Value, cancellationToken);
        if (authorizationResult.IsFailure)
        {
            return (Result<TResponse>.Failure(authorizationResult.Error), null);
        }
    ...
    /// <c>PersonnelFiles.ViewOneTimeDeductions</c> permission (or Admin / IAM super-admin). Requires a completed
    /// employee.</summary>
    protected static async Task<(Result<TResponse>? Failure, PersonnelFile? File)> LoadCompletedEmployeeForOneTimeDeductionReadAsync<TResponse>(
        Guid personnelFileId,
        ITenantContext tenantContext,
        IPersonnelFileAuthorizationService authorizationService,
        IPersonnelFileRepository personnelFileRepository,
        CancellationToken cancellationToken)
    {
        ...
        if (!(await authorizationService.EnsureCanViewOneTimeDeductionsAsync(tenantContext.TenantId.Value, cancellationToken)).IsSuccess)
        {
            return (Result<TResponse>.Failure(PersonnelFileErrors.Forbidden), null);
        }
    ...
    /// <summary>
    /// Manage gate for recurring deductions (REQ-008 D-06): enforces the dedicated
    /// <c>PersonnelFiles.ManageRecurringDeductions</c> permission (or Admin / IAM super-admin). HR-only — there is
    /// NO self-service branch in Fase 1. Item-level optimistic concurrency is enforced by each command's own
    /// If-Match token, so callers pass <see cref="Guid.Empty"/> here. Mirror of the recurring-income gate.
    /// </summary>
    protected static async Task<(Result<TResponse>? Failure, PersonnelFile? File)> LoadForManageRecurringDeductionsAsync<TResponse>(
        Guid personnelFileId,
        Guid concurrencyToken,
        ITenantContext tenantContext,
        IPersonnelFileAuthorizationService authorizationService,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var authorizationResult = await authorizationService.EnsureCanManageRecurringDeductionsAsync(tenantContext.TenantId.Value, cancellationToken);
        if (authorizationResult.IsFailure)
        {
            return (Result<TResponse>.Failure(authorizationResult.Error), null);
        }
    ...
    /// <summary>
    /// Decision/revocation gate for recurring deductions (REQ-008 D-06): enforces the dedicated
    /// <c>PersonnelFiles.AuthorizeRecurringDeductions</c> permission (or IAM super-admin) —
    /// <c>PersonnelFiles.Admin</c> is deliberately EXCLUDED (separation of duties). The double anti-self checks
    /// (subject / registrar) are enforced in the handler.
    /// </summary>
    protected static async Task<(Result<TResponse>? Failure, PersonnelFile? File)> LoadForAuthorizeRecurringDeductionsAsync<TResponse>(
        Guid personnelFileId,
        ITenantContext tenantContext,
        IPersonnelFileAuthorizationService authorizationService,
        IPersonnelFileRepository personnelFileRepository,
        CancellationToken cancellationToken)
    {
        ...
        var authorizationResult = await authorizationService.EnsureCanAuthorizeRecurringDeductionsAsync(tenantContext.TenantId.Value, cancellationToken);
        if (authorizationResult.IsFailure)
        {
            return (Result<TResponse>.Failure(authorizationResult.Error), null);
        }
    ...
    /// <summary>
    /// Read gate for recurring deductions (REQ-008 D-06): enforces the dedicated
    /// <c>PersonnelFiles.ViewRecurringDeductions</c> permission (or Admin / IAM super-admin). Corporate read with
    /// NO self-service branch. Requires a completed employee.
    /// </summary>
    protected static async Task<(Result<TResponse>? Failure, PersonnelFile? File)> LoadCompletedEmployeeForRecurringDeductionReadAsync<TResponse>(
        Guid personnelFileId,
        ITenantContext tenantContext,
        IPersonnelFileAuthorizationService authorizationService,
        IPersonnelFileRepository personnelFileRepository,
        CancellationToken cancellationToken)
    {
        ...
        if (!(await authorizationService.EnsureCanViewRecurringDeductionsAsync(tenantContext.TenantId.Value, cancellationToken)).IsSuccess)
        {
            return (Result<TResponse>.Failure(PersonnelFileErrors.Forbidden), null);
        }
    ...
    /// self-service branch in Fase 1 (P-10). Item-level optimistic concurrency is enforced by each command's own
    /// If-Match token, so callers pass <see cref="Guid.Empty"/> here.
    /// </summary>
    protected static async Task<(Result<TResponse>? Failure, PersonnelFile? File)> LoadForManageOneTimeIncomesAsync<TResponse>(
        Guid personnelFileId,
        Guid concurrencyToken,
        ITenantContext tenantContext,
        IPersonnelFileAuthorizationService authorizationService,
    ...
    /// <summary>
    /// Decision/revocation gate for one-time incomes (REQ-006 P-01/P-02): enforces the dedicated
    /// <c>PersonnelFiles.AuthorizeOneTimeIncomes</c> permission (or IAM super-admin) — <c>PersonnelFiles.Admin</c>
    ...
    /// </summary>
    protected static async Task<(Result<TResponse>? Failure, PersonnelFile? File)> LoadForAuthorizeOneTimeIncomesAsync<TResponse>(
        Guid personnelFileId,
        ITenantContext tenantContext,
        IPersonnelFileAuthorizationService authorizationService,
        IPersonnelFileRepository personnelFileRepository,
        CancellationToken cancellationToken)
    {
        ...
        var authorizationResult = await authorizationService.EnsureCanAuthorizeOneTimeIncomesAsync(tenantContext.TenantId.Value, cancellationToken);
        if (authorizationResult.IsFailure)
        {
            return (Result<TResponse>.Failure(authorizationResult.Error), null);
        }
    ...
    /// <summary>
    /// Read gate for one-time incomes (REQ-006 P-01/P-11): enforces the dedicated
    /// <c>PersonnelFiles.ViewOneTimeIncomes</c> permission (or Admin / IAM super-admin). Corporate read with NO
    /// self-service branch (P-10). Requires a completed employee.
    /// </summary>
    protected static async Task<(Result<TResponse>? Failure, PersonnelFile? File)> LoadCompletedEmployeeForOneTimeIncomeReadAsync<TResponse>(
        Guid personnelFileId,
        ITenantContext tenantContext,
        IPersonnelFileAuthorizationService authorizationService,
        IPersonnelFileRepository personnelFileRepository,
    ...
    /// <summary>
    /// Manage gate for recognitions (REQ-003 D-05): the dedicated <c>PersonnelFiles.ManageRecognitions</c>
    /// permission (or Admin). Used by create/edit and the EN_REVISION annulment — HR-only, no self-service.
    /// Item-level optimistic concurrency is enforced by each command's own If-Match token, so callers pass
    /// <see cref="Guid.Empty"/> here.
    /// </summary>
    protected static async Task<(Result<TResponse>? Failure, PersonnelFile? File)> LoadForManageRecognitionsAsync<TResponse>(
        Guid personnelFileId,
        Guid concurrencyToken,
        ITenantContext tenantContext,
        IPersonnelFileAuthorizationService authorizationService,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var authorizationResult = await authorizationService.EnsureCanManageRecognitionsAsync(tenantContext.TenantId.Value, cancellationToken);
        if (authorizationResult.IsFailure)
        {
            return (Result<TResponse>.Failure(authorizationResult.Error), null);
        }
    ...
    /// <summary>
    /// Gate for the AUTHORIZER actions on a recognition — the single decision (APLICAR/RECHAZAR) and the
    /// revocation of an APLICADA (REQ-003 D-05): the dedicated <c>PersonnelFiles.AuthorizeRecognitions</c>
    /// permission or the IAM super-admin. <c>PersonnelFiles.Admin</c> is deliberately EXCLUDED (separation of
    /// duties, mirrors AuthorizeRetirement). The double anti-self-approval check is applied by the handler.
    /// </summary>
    protected static async Task<(Result<TResponse>? Failure, PersonnelFile? File)> LoadForAuthorizeRecognitionsAsync<TResponse>(
        Guid personnelFileId,
        ITenantContext tenantContext,
        IPersonnelFileAuthorizationService authorizationService,
        IPersonnelFileRepository personnelFileRepository,
        CancellationToken cancellationToken)
    {
        ...
        var authorizationResult = await authorizationService.EnsureCanAuthorizeRecognitionsAsync(tenantContext.TenantId.Value, cancellationToken);
        if (authorizationResult.IsFailure)
        {
            return (Result<TResponse>.Failure(authorizationResult.Error), null);
        }
...
internal abstract class PersonnelFileEmployeeReadQueryHandlerBase : PersonnelFileEmployeeCommandHandlerBase
{
    ...
    /// <summary>
    /// Read gate for recognition sub-resources (REQ-003 D-05/D-13): the caller has the dedicated
    /// <c>ViewRecognitions</c> permission (or Admin), OR the caller is the employee reading their own
    /// recognitions (self-service). Without the permission and without being the owner the caller gets 403
    /// (no masking). Requires a completed employee. Returns <c>RestrictToApplied</c> = true when the caller was
    /// authorized only by the self branch, so the query handler surfaces exclusively APLICADA records to the
    /// employee (the trays and intermediate states never travel to them — D-13).
    /// </summary>
    protected static async Task<(Result<TResponse>? Failure, PersonnelFile? File, bool RestrictToApplied)> LoadCompletedEmployeeForRecognitionReadAsync<TResponse>(
        Guid personnelFileId,
        ITenantContext tenantContext,
        IPersonnelFileAuthorizationService authorizationService,
        ICurrentUserService currentUserService,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var canViewByRole = (await authorizationService.EnsureCanViewRecognitionsAsync(tenantContext.TenantId.Value, cancellationToken)).IsSuccess;
        if (!canViewByRole)
        {
            var isSelf = personnelFile.LinkedUserPublicId is { } linkedUserPublicId
                && Guid.TryParse(currentUserService.UserId, out var callerUserPublicId)
