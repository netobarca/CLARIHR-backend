...
public sealed record SearchCompanyTypesQuery(
    bool? IsActive,
    string? Search,
    int PageNumber = 1,
    int PageSize = OrgStructureCatalogValidationRules.DefaultPageSize)
    : IQuery<PagedResponse<CompanyTypeCatalogItemResponse>>;

public sealed record GetCompanyTypeByIdQuery(Guid CompanyTypeId)
    : IQuery<CompanyTypeCatalogItemResponse>;

public sealed record CreateCompanyTypeCommand(
    string Code,
    string Name,
    string? Description,
    int SortOrder)
    : ICommand<CompanyTypeCatalogItemResponse>;

public sealed record UpdateCompanyTypeCommand(
    Guid CompanyTypeId,
    string Code,
    string Name,
    string? Description,
...
public sealed record ActivateCompanyTypeCommand(
    Guid CompanyTypeId,
    Guid ConcurrencyToken)
    : ICommand<CompanyTypeCatalogItemResponse>;

public sealed record InactivateCompanyTypeCommand(
    Guid CompanyTypeId,
    Guid ConcurrencyToken)
    : ICommand<CompanyTypeCatalogItemResponse>;
...
internal sealed class SearchCompanyTypesQueryValidator : AbstractValidator<SearchCompanyTypesQuery>
{
    public SearchCompanyTypesQueryValidator()
    {
...
internal sealed class GetCompanyTypeByIdQueryValidator : AbstractValidator<GetCompanyTypeByIdQuery>
{
    public GetCompanyTypeByIdQueryValidator()
    {
        RuleFor(query => query.CompanyTypeId).NotEmpty();
    }
}

internal sealed class CreateCompanyTypeCommandValidator : AbstractValidator<CreateCompanyTypeCommand>
{
    ...
    private void ApplyCatalogRules()
    {
...
internal sealed class UpdateCompanyTypeCommandValidator : AbstractValidator<UpdateCompanyTypeCommand>
{
    public UpdateCompanyTypeCommandValidator()
    {
        RuleFor(command => command.CompanyTypeId).NotEmpty();
        RuleFor(command => command.Code)
            .NotEmpty()
            .MaximumLength(50)
            .Must(OrgStructureCatalogValidationRules.IsValidCode)
...
internal sealed class ActivateCompanyTypeCommandValidator : AbstractValidator<ActivateCompanyTypeCommand>
{
    public ActivateCompanyTypeCommandValidator()
    {
        RuleFor(command => command.CompanyTypeId).NotEmpty();
        RuleFor(command => command.ConcurrencyToken).NotEmpty();
    }
...
internal sealed class InactivateCompanyTypeCommandValidator : AbstractValidator<InactivateCompanyTypeCommand>
{
    public InactivateCompanyTypeCommandValidator()
    {
        RuleFor(command => command.CompanyTypeId).NotEmpty();
        RuleFor(command => command.ConcurrencyToken).NotEmpty();
    }
...
{
    public async Task<Result<PagedResponse<CompanyTypeCatalogItemResponse>>> Handle(
        SearchCompanyTypesQuery query,
        CancellationToken cancellationToken)
    {
        var authResult = await authorizationService.EnsureAccountScopeAsync(cancellationToken);
        ...
        return Result<PagedResponse<CompanyTypeCatalogItemResponse>>.Success(response);
    }
}

internal sealed class GetCompanyTypeByIdQueryHandler(
    IOrgStructureCatalogAuthorizationService authorizationService,
    IOrgStructureCatalogRepository repository)
    : IQueryHandler<GetCompanyTypeByIdQuery, CompanyTypeCatalogItemResponse>
{
    public async Task<Result<CompanyTypeCatalogItemResponse>> Handle(
        GetCompanyTypeByIdQuery query,
        CancellationToken cancellationToken)
    {
        var authResult = await authorizationService.EnsureAccountScopeAsync(cancellationToken);
        if (authResult.IsFailure)
        {
            return Result<CompanyTypeCatalogItemResponse>.Failure(authResult.Error);
        }

        var entity = await repository.GetCompanyTypeByIdAsync(query.CompanyTypeId, cancellationToken);
        if (entity is null || entity.OwnerUserPublicId != authResult.Value)
        {
            return Result<CompanyTypeCatalogItemResponse>.Failure(OrgStructureCatalogErrors.CatalogNotFound);
        }

        var response = await repository.GetCompanyTypeResponseByIdAsync(query.CompanyTypeId, cancellationToken)
            ?? throw new InvalidOperationException("Company type response could not be resolved.");

        return Result<CompanyTypeCatalogItemResponse>.Success(response);
    }
}
...
internal sealed class CreateCompanyTypeCommandHandler(
    ...
    : ICommandHandler<CreateCompanyTypeCommand, CompanyTypeCatalogItemResponse>
{
    public async Task<Result<CompanyTypeCatalogItemResponse>> Handle(
        CreateCompanyTypeCommand command,
        CancellationToken cancellationToken)
    {
        var authResult = await authorizationService.EnsureAccountScopeAsync(cancellationToken);
        if (authResult.IsFailure)
        ...
        if (await repository.CompanyTypeCodeExistsAsync(authResult.Value, normalizedCode, excludingId: null, cancellationToken))
        {
            return Result<CompanyTypeCatalogItemResponse>.Failure(OrgStructureCatalogErrors.CatalogCodeConflict);
        }

        var entity = CompanyTypeCatalogItem.Create(authResult.Value, command.Code, command.Name, command.Description, command.SortOrder);

        await using var transaction = await unitOfWork.BeginTransactionAsync(cancellationToken);
        try
        {
            repository.AddCompanyType(entity);
            _ = await unitOfWork.SaveChangesAsync(cancellationToken);

            var response = await repository.GetCompanyTypeResponseByIdAsync(entity.PublicId, cancellationToken)
                ?? throw new InvalidOperationException("Company type response could not be resolved after creation.");
            ...
            if (tenantContext.TenantId.HasValue)
            {
                await auditService.LogForTenantAsync(
                    ...
                    new AuditLogEntry(
                        AuditEventTypes.CompanyTypeCatalogItemCreated,
                        AuditEntityTypes.CompanyTypeCatalogItem,
                        entity.PublicId,
                        entity.Code,
                        AuditActions.Create,
                        $"Created company type {entity.Code}.",
                        After: response),
                    cancellationToken);
                _ = await unitOfWork.SaveChangesAsync(cancellationToken);
            }
...
internal sealed class UpdateCompanyTypeCommandHandler(
    ...
    ITenantContext tenantContext,
    IUnitOfWork unitOfWork)
    : ICommandHandler<UpdateCompanyTypeCommand, CompanyTypeCatalogItemResponse>
{
    public async Task<Result<CompanyTypeCatalogItemResponse>> Handle(
        UpdateCompanyTypeCommand command,
        CancellationToken cancellationToken)
    {
        var authResult = await authorizationService.EnsureAccountScopeAsync(cancellationToken);
        if (authResult.IsFailure)
        ...
        var entity = await repository.GetCompanyTypeByIdAsync(command.CompanyTypeId, cancellationToken);
        if (entity is null || entity.OwnerUserPublicId != authResult.Value)
        {
            return Result<CompanyTypeCatalogItemResponse>.Failure(OrgStructureCatalogErrors.CatalogNotFound);
        }
        ...
        {
            return Result<CompanyTypeCatalogItemResponse>.Failure(OrgStructureCatalogErrors.ConcurrencyConflict);
        }

        var normalizedCode = command.Code.Trim().ToUpperInvariant();
        if (await repository.CompanyTypeCodeExistsAsync(entity.OwnerUserPublicId, normalizedCode, entity.Id, cancellationToken))
        {
            return Result<CompanyTypeCatalogItemResponse>.Failure(OrgStructureCatalogErrors.CatalogCodeConflict);
        }

        var before = await repository.GetCompanyTypeResponseByIdAsync(entity.PublicId, cancellationToken)
            ?? throw new InvalidOperationException("Company type response could not be resolved before update.");
        ...
        try
        {
            ...
            var after = await repository.GetCompanyTypeResponseByIdAsync(entity.PublicId, cancellationToken)
                ?? throw new InvalidOperationException("Company type response could not be resolved after update.");
            ...
            if (tenantContext.TenantId.HasValue)
            {
                await auditService.LogForTenantAsync(
                    tenantContext.TenantId.Value,
                    new AuditLogEntry(
                        AuditEventTypes.CompanyTypeCatalogItemUpdated,
                        AuditEntityTypes.CompanyTypeCatalogItem,
                        entity.PublicId,
                        entity.Code,
                        AuditActions.Update,
                        $"Updated company type {entity.Code}.",
                        Before: before,
                        After: after),
                    cancellationToken);
                _ = await unitOfWork.SaveChangesAsync(cancellationToken);
            ...
            await transaction.CommitAsync(cancellationToken);
            return Result<CompanyTypeCatalogItemResponse>.Success(after);
        }
        catch
        {
            await transaction.RollbackAsync(cancellationToken);
...
internal sealed class ActivateCompanyTypeCommandHandler(
    IOrgStructureCatalogAuthorizationService authorizationService,
    IOrgStructureCatalogRepository repository,
    IAuditService auditService,
    ITenantContext tenantContext,
    IUnitOfWork unitOfWork)
    : ICommandHandler<ActivateCompanyTypeCommand, CompanyTypeCatalogItemResponse>
{
    public async Task<Result<CompanyTypeCatalogItemResponse>> Handle(
        ActivateCompanyTypeCommand command,
        CancellationToken cancellationToken)
    {
        var authResult = await authorizationService.EnsureAccountScopeAsync(cancellationToken);
        if (authResult.IsFailure)
        ...
        var before = await repository.GetCompanyTypeResponseByIdAsync(entity.PublicId, cancellationToken)
            ?? throw new InvalidOperationException("Company type response could not be resolved before activation.");
        ...
        try
        {
            ...
            var after = await repository.GetCompanyTypeResponseByIdAsync(entity.PublicId, cancellationToken)
                ?? throw new InvalidOperationException("Company type response could not be resolved after activation.");
            ...
            if (tenantContext.TenantId.HasValue)
            {
                await auditService.LogForTenantAsync(
                    tenantContext.TenantId.Value,
                    new AuditLogEntry(
                        AuditEventTypes.CompanyTypeCatalogItemActivated,
                        AuditEntityTypes.CompanyTypeCatalogItem,
                        entity.PublicId,
                        entity.Code,
                        AuditActions.Reactivate,
                        $"Activated company type {entity.Code}.",
                        Before: before,
                        After: after),
                    cancellationToken);
                _ = await unitOfWork.SaveChangesAsync(cancellationToken);
...
internal sealed class InactivateCompanyTypeCommandHandler(
    IOrgStructureCatalogAuthorizationService authorizationService,
    IOrgStructureCatalogRepository repository,
    IAuditService auditService,
    ITenantContext tenantContext,
    IUnitOfWork unitOfWork)
    : ICommandHandler<InactivateCompanyTypeCommand, CompanyTypeCatalogItemResponse>
{
    public async Task<Result<CompanyTypeCatalogItemResponse>> Handle(
        InactivateCompanyTypeCommand command,
        CancellationToken cancellationToken)
    {
        var authResult = await authorizationService.EnsureAccountScopeAsync(cancellationToken);
        if (authResult.IsFailure)
        ...
        if (await repository.HasCompaniesUsingCompanyTypeAsync(entity.Id, cancellationToken))
        {
            return Result<CompanyTypeCatalogItemResponse>.Failure(OrgStructureCatalogErrors.CatalogInUse);
        }

        var before = await repository.GetCompanyTypeResponseByIdAsync(entity.PublicId, cancellationToken)
            ?? throw new InvalidOperationException("Company type response could not be resolved before inactivation.");
        ...
        try
        {
            ...
            var after = await repository.GetCompanyTypeResponseByIdAsync(entity.PublicId, cancellationToken)
                ?? throw new InvalidOperationException("Company type response could not be resolved after inactivation.");
            ...
            if (tenantContext.TenantId.HasValue)
            {
                await auditService.LogForTenantAsync(
                    tenantContext.TenantId.Value,
                    new AuditLogEntry(
                        AuditEventTypes.CompanyTypeCatalogItemInactivated,
                        AuditEntityTypes.CompanyTypeCatalogItem,
                        entity.PublicId,
                        entity.Code,
                        AuditActions.Deactivate,
                        $"Inactivated company type {entity.Code}.",
                        Before: before,
                        After: after),
                    cancellationToken);
                _ = await unitOfWork.SaveChangesAsync(cancellationToken);
