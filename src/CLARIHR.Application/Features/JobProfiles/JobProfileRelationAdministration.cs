    ...
    : ICommandHandler<AddJobProfileRelationCommand, JobProfileRelationResponse>
{
    public async Task<Result<JobProfileRelationResponse>> Handle(AddJobProfileRelationCommand command, CancellationToken cancellationToken)
    {
        ...
        catch (InvalidOperationException ex)
        {
            ...
            return Result<JobProfileRelationResponse>.Failure(JobProfileErrors.FromDomainException(ex));
        }
        catch
        {
            await transaction.RollbackAsync(cancellationToken);
...
internal sealed class RemoveJobProfileRelationCommandHandler(
    ...
    ITenantContext tenantContext,
    IUnitOfWork unitOfWork)
    : ICommandHandler<RemoveJobProfileRelationCommand, ChildDeletionResult>
{
    public async Task<Result<ChildDeletionResult>> Handle(RemoveJobProfileRelationCommand command, CancellationToken cancellationToken)
    {
        if (!tenantContext.TenantId.HasValue)
        {
        ...
        if (relation is null)
        {
            return Result<ChildDeletionResult>.Failure(JobProfileErrors.RelationNotFound);
        }
