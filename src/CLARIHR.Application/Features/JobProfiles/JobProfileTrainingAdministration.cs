    ...
    : ICommandHandler<AddJobProfileTrainingCommand, JobProfileTrainingResponse>
{
    public async Task<Result<JobProfileTrainingResponse>> Handle(AddJobProfileTrainingCommand command, CancellationToken cancellationToken)
    {
        ...
        catch (InvalidOperationException ex)
        {
            ...
            return Result<JobProfileTrainingResponse>.Failure(JobProfileErrors.FromDomainException(ex));
        }
        catch
        {
            await transaction.RollbackAsync(cancellationToken);
...
internal sealed class RemoveJobProfileTrainingCommandHandler(
    ...
    ITenantContext tenantContext,
    IUnitOfWork unitOfWork)
    : ICommandHandler<RemoveJobProfileTrainingCommand, ChildDeletionResult>
{
    public async Task<Result<ChildDeletionResult>> Handle(RemoveJobProfileTrainingCommand command, CancellationToken cancellationToken)
    {
        if (!tenantContext.TenantId.HasValue)
        {
        ...
        if (training is null)
        {
            return Result<ChildDeletionResult>.Failure(JobProfileErrors.TrainingNotFound);
        }
