...
public sealed record EducationCatalogItemPatchOperation(
    string Op,
    string Path,
    string? From,
    JsonElement? Value);

public sealed record PatchEducationCatalogItemCommand(
    EducationCatalogType CatalogType,
    Guid Id,
    Guid ConcurrencyToken,
    IReadOnlyCollection<EducationCatalogItemPatchOperation> Operations)
    : ICommand<EducationCatalogItemResponse>;

internal sealed class PatchEducationCatalogItemCommandValidator
    : AbstractValidator<PatchEducationCatalogItemCommand>
{
    public PatchEducationCatalogItemCommandValidator()
    {
...
internal sealed class PatchEducationCatalogItemCommandHandler(
    IPlatformAuthorizationService authorizationService,
    IEducationCatalogRepository repository,
    IUnitOfWork unitOfWork)
    : ICommandHandler<PatchEducationCatalogItemCommand, EducationCatalogItemResponse>
{
    public async Task<Result<EducationCatalogItemResponse>> Handle(
        PatchEducationCatalogItemCommand command,
        CancellationToken cancellationToken)
    {
        ...
        var state = EducationCatalogItemPatchState.From(entity);

        var applied = EducationCatalogItemPatchApplier.Apply(command.Operations, state);
        if (applied.IsFailure)
        {
            return Result<EducationCatalogItemResponse>.Failure(applied.Error);
        }

        var validation = EducationCatalogItemPatchApplier.Validate(state);
        if (validation.IsFailure)
        {
            return Result<EducationCatalogItemResponse>.Failure(validation.Error);
        }

        if (!state.HasMutation)
        {
            var unchanged = await repository.GetResponseByIdAsync(command.CatalogType, command.Id, cancellationToken);
            return unchanged is null
                ? Result<EducationCatalogItemResponse>.Failure(EducationCatalogErrors.NotFound)
                : Result<EducationCatalogItemResponse>.Success(unchanged);
        }
        ...
        // Scalar-only patch: code/name/sortOrder are the editable fields, re-normalized + re-validated
        // by Update exactly as the PUT path does. Status transitions go through /activate and /inactivate;
        // the catalog type is the route discriminator and is not patchable.
...
internal sealed class EducationCatalogItemPatchState
{
    ...
    public static EducationCatalogItemPatchState From(EducationCatalogItem entity) =>
        new()
        {
            Code = entity.Code,
            Name = entity.Name,
...
internal sealed class EducationCatalogItemPatchValueException(string path, string message) : Exception(message)
{
    public string Path { get; } = path;
}

internal static class EducationCatalogItemPatchApplier
{
    ...
    public static Result Apply(IReadOnlyCollection<EducationCatalogItemPatchOperation> operations, EducationCatalogItemPatchState state)
    {
        foreach (var operation in operations)
        {
            ...
            catch (EducationCatalogItemPatchValueException exception)
            {
    ...
    public static Result Validate(EducationCatalogItemPatchState state)
    {
    ...
    private static Result ApplyOperation(
        ...
        string property,
        JsonElement? value,
        EducationCatalogItemPatchState state,
        string path)
    {
        ...
        if (IsSegment(property, "catalogType"))
        {
            return ValidationFailure(path, "The catalog type cannot be patched.");
        }
    ...
    private static string ReadRequiredString(JsonElement? value, string path)
    {
        if (IsNull(value))
        {
            throw new EducationCatalogItemPatchValueException(path, "Value is required.");
        }

        return value!.Value.ValueKind == JsonValueKind.String
            ? value.Value.GetString() ?? string.Empty
            : throw new EducationCatalogItemPatchValueException(path, "Value must be a string.");
    }
    ...
    private static int ReadRequiredInt(JsonElement? value, string path)
    {
        ...
        return value!.Value.ValueKind == JsonValueKind.Number && value.Value.TryGetInt32(out var parsed)
            ? parsed
            : throw new EducationCatalogItemPatchValueException(path, "Value must be an integer.");
    }
