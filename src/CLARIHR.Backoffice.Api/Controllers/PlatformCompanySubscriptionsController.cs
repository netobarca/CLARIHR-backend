    ...
    IQueryDispatcher queryDispatcher) : ControllerBase
{
    ...
    public async Task<ActionResult<PlatformCompanySubscriptionResponse>> GetCurrent(
    ...
    public async Task<ActionResult<PlatformCompanySubscriptionResponse>> Replace(
        Guid companyPublicId,
        [FromBody] ReplacePlatformCompanySubscriptionRequest request,
        CancellationToken cancellationToken = default)
    {
        var result = await commandDispatcher.SendAsync(
            new ReplacePlatformCompanySubscriptionCommand(companyPublicId, request.CommercialPlanId),
            cancellationToken);

        return this.ToActionResult(result);
    }

    public sealed record ReplacePlatformCompanySubscriptionRequest(Guid CommercialPlanId);
}
