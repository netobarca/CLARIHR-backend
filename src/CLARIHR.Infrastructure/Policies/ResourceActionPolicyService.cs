...
internal sealed class ResourceActionPolicyService : IResourceActionPolicyService
{
    public AllowedActionsResponse Evaluate(ResourceActionContext context)
    {
        ...
        var canEdit = context.SupportsEdit;
        if (context.IsSystem)
        {
            canEdit = false;
        ...
        var canDelete = context.SupportsDelete && !context.IsSystem && !context.HasDependencies;
        if (!canDelete)
        {
        ...
        var canArchive = context.SupportsArchive && context.IsActive;
        if (!canArchive && context.SupportsArchive)
        {
            reasons.Add(context.IsActive
                ? "The record cannot be archived due to current restrictions."
                : "The record is already inactive/archived.");
        }

        var canActivate = context.SupportsActivate && !context.IsActive;
        ...
        var canInactivate = context.SupportsInactivate && context.IsActive && !context.HasDependencies;
