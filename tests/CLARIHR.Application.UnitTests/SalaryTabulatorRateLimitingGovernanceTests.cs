...
/// <summary>
/// Salary Tabulator rate-limit guardrail (ST-A; drift-proof by construction; mirrors the
/// <see cref="CostCenterRateLimitingGovernanceTests"/> template). The unbounded-cost salary-line export
/// surfaces salary PII, so it MUST declare an <c>[EnableRateLimiting]</c> policy — a newly added or
/// renamed heavy endpoint cannot ship without a per-tenant abuse guard. Pure reflection over the
/// handler-gated SalaryTabulator controller — family-scoped, aggregated single assert, loud zero-match
/// sentinel.
///
...
/// used by <see cref="CostCenterRateLimitingGovernanceTests"/>).</para>
/// </summary>
public sealed class SalaryTabulatorRateLimitingGovernanceTests
{
    private static readonly Assembly ApiAssembly = typeof(AuthorizationPolicySetAttribute).Assembly;

    private static readonly Regex SalaryTabulatorFamilyRegex =
        new(@"^SalaryTabulator", RegexOptions.Compiled);
    ...
    {
        SalaryTabulatorRateLimitPolicies.Search,
        SalaryTabulatorRateLimitPolicies.Export,
    };
    ...
    private static IReadOnlyList<MethodInfo> FamilyActions() =>
        ApiAssembly.GetTypes()
            .Where(static type =>
                ...
                SalaryTabulatorFamilyRegex.IsMatch(type.Name))
    ...
    /// <summary>
    /// Inv-R1 — every unbounded-cost generator (the salary PII export) declares a canonical rate-limit
    /// policy. Catches a heavy endpoint shipping with no abuse guard.
    /// </summary>
    [Fact]
    public void EveryHeavySalaryTabulatorEndpoint_DeclaresRateLimitPolicy()
    {
        ...
        Assert.True(
            unguarded.Length == 0,
            "Finding ST-A: unbounded-cost Salary Tabulator generators must declare " +
            "[EnableRateLimiting(SalaryTabulatorRateLimitPolicies.*)] or they have no per-tenant " +
            "abuse guard over salary PII. Missing/invalid on:\n  " + string.Join("\n  ", unguarded));
    }

    /// <summary>
    /// Inv-R2 — every <c>[EnableRateLimiting]</c> references a canonical constant from
    /// <see cref="SalaryTabulatorRateLimitPolicies"/> (no typo / orphan policy).
    /// </summary>
    [Fact]
    public void EveryRateLimitMarker_ReferencesCanonicalPolicy()
    {
        ...
        Assert.True(
            invalid.Length == 0,
            "[EnableRateLimiting] must reference a constant from SalaryTabulatorRateLimitPolicies. " +
            "Offending:\n  " + string.Join("\n  ", invalid));
    }
    ...
    public void BothRateLimitPolicies_AreAppliedToAtLeastOneEndpoint()
    {
        ...
        Assert.Contains(SalaryTabulatorRateLimitPolicies.Export, applied);
        Assert.Contains(SalaryTabulatorRateLimitPolicies.Search, applied);
    }
}
