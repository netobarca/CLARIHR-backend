...
/// <summary>
/// OU-005 guardrail (defense-in-depth) for the Org Units list endpoint; mirrors
/// <see cref="CostCenterPaginationGuardrailsTests"/>. Every paginated <c>pageSize</c> on an Org Units
/// controller MUST constrain its bounds at the controller boundary with
/// <c>[Range(1, OrgUnitValidationRules.MaxPageSize)]</c> — the same bounds as the handler
/// FluentValidation (<c>SearchOrgUnitsQueryValidator.InclusiveBetween(1, MaxPageSize)</c>) — instead of
...
/// </summary>
public sealed class OrgUnitPaginationGuardrailsTests
{
    ...
    private static readonly Regex OrgUnitFamilyRegex =
        new(@"^OrganizationUnits", RegexOptions.Compiled);
    ...
    [Fact]
    public void OrgUnitFamily_ExposesPaginatedActions()
    {
        ...
        Assert.True(
            pageSizeParams > 0,
            "Expected at least one 'pageSize' parameter across the Org Units controller family. " +
            "Zero means the namespace/name filter drifted and the guardrail would silently pass.");
    }

    [Fact]
    public void EveryOrgUnitPageSize_DeclaresRangeMatchingTheHandlerValidator()
    {
        ...
        foreach (var action in FamilyActions())
        {
            foreach (var parameter in action.GetParameters())
            {
                ...
                if (Convert.ToInt32(range.Maximum) != OrgUnitValidationRules.MaxPageSize)
                {
                    violations.Add(
                        $"{qualifiedName}: [Range] Maximum is {range.Maximum}, expected " +
                        $"{OrgUnitValidationRules.MaxPageSize} (OrgUnitValidationRules.MaxPageSize).");
                }
            }
        }
        ...
        Assert.True(
            violations.Count == 0,
            "Finding OU-005 (defense-in-depth): every paginated 'pageSize' on an Org Units controller " +
            "must declare [Range(1, OrgUnitValidationRules.MaxPageSize)] at the controller boundary, " +
