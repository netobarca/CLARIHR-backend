...
/// <summary>
/// Competency Framework rate-limit guardrail (drift-proof by construction; mirrors the §X-RATE
/// <see cref="RateLimitingGovernanceTests"/> Position Slots template and
/// <see cref="PersonnelFileRateLimitingGovernanceTests"/>). The unbounded-cost competency-matrix
/// export MUST declare an <c>[EnableRateLimiting]</c> policy, so a newly added or renamed heavy
/// endpoint cannot ship without a per-tenant abuse guard. Pure reflection over the three
/// CompetencyFramework controllers — family-scoped, aggregated single assert, loud zero-match
/// sentinel.
///
...
/// </summary>
public sealed class CompetencyFrameworkRateLimitingGovernanceTests
{
    ...
    {
        CompetencyFrameworkRateLimitPolicies.Search,
        CompetencyFrameworkRateLimitPolicies.Export,
    };
    ...
    /// <summary>
    /// Inv-R1 — every unbounded-cost generator (the competency-matrix export) declares a canonical
    /// rate-limit policy. Catches a heavy endpoint shipping with no abuse guard.
    /// </summary>
    [Fact]
    public void EveryHeavyCompetencyFrameworkEndpoint_DeclaresRateLimitPolicy()
    {
        ...
        Assert.True(
            unguarded.Length == 0,
            "Finding §X-RATE: unbounded-cost Competency Framework generators must declare " +
            "[EnableRateLimiting(CompetencyFrameworkRateLimitPolicies.*)] or they have no per-tenant " +
            "abuse guard. Missing/invalid on:\n  " + string.Join("\n  ", unguarded));
    }

    /// <summary>
    /// Inv-R2 — every <c>[EnableRateLimiting]</c> references a canonical constant from
    /// <see cref="CompetencyFrameworkRateLimitPolicies"/> (no typo / orphan policy).
    /// </summary>
    [Fact]
    public void EveryRateLimitMarker_ReferencesCanonicalPolicy()
    {
        ...
        Assert.True(
            invalid.Length == 0,
            "[EnableRateLimiting] must reference a constant from CompetencyFrameworkRateLimitPolicies. " +
            "Offending:\n  " + string.Join("\n  ", invalid));
    }
    ...
    public void BothRateLimitPolicies_AreAppliedToAtLeastOneEndpoint()
    {
        ...
        Assert.Contains(CompetencyFrameworkRateLimitPolicies.Export, applied);
        Assert.Contains(CompetencyFrameworkRateLimitPolicies.Search, applied);
    }
}
