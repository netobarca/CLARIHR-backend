...
/// <summary>
/// Cost Centers rate-limit guardrail (drift-proof by construction; mirrors the §X-RATE
/// <see cref="LegalRepresentativeRateLimitingGovernanceTests"/> template). The unbounded-cost
/// cost-centers report export MUST declare an <c>[EnableRateLimiting]</c> policy, so a newly added
/// or renamed heavy endpoint cannot ship without a per-tenant abuse guard. Pure reflection over the
/// autonomous CostCenters controller — family-scoped, aggregated single assert, loud zero-match
/// sentinel.
///
/// <para>The ASP.NET endpoint attributes (<c>EnableRateLimitingAttribute</c>,
/// <c>HttpGetAttribute</c>) are detected by <b>simple type name</b> and read via reflection — they
/// are not cleanly compile-referenceable from this non-Web SDK test project (same
/// framework-type-avoidance technique used by <see cref="LegalRepresentativeRateLimitingGovernanceTests"/>).</para>
/// </summary>
public sealed class CostCenterRateLimitingGovernanceTests
{
    ...
    {
        CostCenterRateLimitPolicies.Search,
        CostCenterRateLimitPolicies.Export,
    };
    ...
    private static IReadOnlyList<MethodInfo> FamilyActions() =>
        ApiAssembly.GetTypes()
            .Where(static type =>
                ...
                CostCenterFamilyRegex.IsMatch(type.Name))
    ...
    /// </summary>
    [Fact]
    public void EveryHeavyCostCenterEndpoint_DeclaresRateLimitPolicy()
    {
        ...
        Assert.True(
            unguarded.Length == 0,
            "Finding §X-RATE: unbounded-cost Cost Centers generators must declare " +
            "[EnableRateLimiting(CostCenterRateLimitPolicies.*)] or they have no per-tenant " +
            "abuse guard. Missing/invalid on:\n  " + string.Join("\n  ", unguarded));
    }

    /// <summary>
    /// Inv-R2 — every <c>[EnableRateLimiting]</c> references a canonical constant from
    /// <see cref="CostCenterRateLimitPolicies"/> (no typo / orphan policy).
    /// </summary>
    [Fact]
    public void EveryRateLimitMarker_ReferencesCanonicalPolicy()
    {
        ...
        Assert.True(
            invalid.Length == 0,
            "[EnableRateLimiting] must reference a constant from CostCenterRateLimitPolicies. " +
            "Offending:\n  " + string.Join("\n  ", invalid));
    }
    ...
    public void BothRateLimitPolicies_AreAppliedToAtLeastOneEndpoint()
    {
        ...
        Assert.Contains(CostCenterRateLimitPolicies.Export, applied);
        Assert.Contains(CostCenterRateLimitPolicies.Search, applied);
    }
}
