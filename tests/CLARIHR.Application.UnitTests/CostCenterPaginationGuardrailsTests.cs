...
/// <summary>
/// CC2 guardrail (defense-in-depth) for the Cost Centers list endpoint; mirrors
/// <see cref="CompanyUserPaginationGuardrailsTests"/>. Every paginated <c>pageSize</c> on a
/// CostCenters controller MUST constrain its bounds at the controller boundary with
/// <c>[Range(1, CostCenterValidationRules.MaxPageSize)]</c> — the same bounds as the handler
/// FluentValidation (<c>SearchCostCentersQueryValidator</c>) — instead of relying solely on the
/// handler validator. Structural pattern (namespace + name regex), not a hand-maintained list, so a
/// new family controller cannot silently regress.
/// </summary>
public sealed class CostCenterPaginationGuardrailsTests
{
    ...
    [Fact]
    public void CostCenterFamily_ExposesPaginatedActions()
    {
        ...
        Assert.True(
            pageSizeParams > 0,
            "Expected at least one 'pageSize' parameter across the CostCenters controller family. " +
            "Zero means the namespace/name filter drifted and the guardrail would silently pass.");
    }

    [Fact]
    public void EveryCostCenterPageSize_DeclaresRangeMatchingTheHandlerValidator()
    {
        ...
        foreach (var action in FamilyActions())
        {
            foreach (var parameter in action.GetParameters())
            {
                ...
                if (Convert.ToInt32(range.Maximum) != CostCenterValidationRules.MaxPageSize)
                {
                    violations.Add(
                        $"{qualifiedName}: [Range] Maximum is {range.Maximum}, expected " +
                        $"{CostCenterValidationRules.MaxPageSize} (CostCenterValidationRules.MaxPageSize).");
                }
            }
        }
        ...
        Assert.True(
            violations.Count == 0,
            "Finding CC2 (defense-in-depth): every paginated 'pageSize' on a CostCenters controller " +
            "must declare [Range(1, CostCenterValidationRules.MaxPageSize)] at the controller boundary, " +
