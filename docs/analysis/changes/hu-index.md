...
|---|---|---|---|---|---|---|---|
| HU-2026-04-13-02 | Preview de validacion previa a finalize de expediente | Personnel Files / Employment | Implementada | 2026-04-13 | `docs/analysis/changes/HU-2026-04-13-02.md` | `docs/technical/api/openapi.yaml`, `docs/technical/api/endpoint-reference.md`, `docs/business/current-system-business-flows.md` | Agrega `POST /api/v1/personnel-files/{id}/finalize/preview` con `isEligible + issues` y reutiliza la logica de elegibilidad del `finalize` para evitar divergencia de reglas. |
| HU-2026-04-13-01 | Finalizacion de expediente con creacion de cuenta opcional | Personnel Files / Company Users | Implementada | 2026-04-13 | `docs/analysis/changes/HU-2026-04-13-01.md` | `docs/business/current-system-business-flows.md`, `docs/technical/api/endpoint-reference.md`, `docs/technical/api/openapi.yaml` | `finalize` ahora acepta `createUserAccount` y permite completar expediente sin aprovisionar usuario, manteniendo default `true` para clientes existentes. |
| HU-2026-04-12-01 | Catalogos de referencia del expediente personal para El Salvador | Personnel Files / Locations / Onboarding | Implementada | 2026-04-12 | `docs/analysis/changes/HU-2026-04-12-01.md` | `docs/technical/api/openapi.yaml`, `docs/technical/api/endpoint-reference.md`, `docs/business/current-system-business-flows.md`, `docs/analysis/current-state/architecture-analysis.md` | Migra campos personales e identificaciones a codigos de catalogo, agrega catalogos read-only por compania, siembra `SV` (14/44), limpia datos legacy del expediente y sincroniza seed territorial con onboarding. |
| HU-2026-04-11-01 | Finalizacion de expediente y aprovisionamiento automatico de usuario | Personnel Files / Position Slots / Company Users / Auth | Implementada | 2026-04-11 | `docs/analysis/changes/HU-2026-04-11-01.md` | `docs/business/current-system-business-flows.md`, `docs/analysis/current-state/architecture-analysis.md`, `docs/analysis/current-state/security-analysis.md`, `docs/analysis/current-state/testing-analysis.md`, `docs/technical/api/endpoint-reference.md`, `docs/technical/api/openapi.yaml` | Reemplaza `hire` por `finalize`, agrega estado `Draft/Completed`, rol por plaza, vinculacion opcional expediente-usuario e invitacion de activacion con contrasena temporal. |
| HU-BILL-012 | Rebalance de planes `FREE` y `MASTER` | Platform Billing / Commercial Catalog / Owner Subscriptions | Implementada | 2026-04-07 | `docs/analysis/changes/HU-BILL-012.md` | `docs/business/current-system-business-flows.md`, `docs/analysis/current-state/architecture-analysis.md`, `docs/analysis/current-state/security-analysis.md`, `docs/analysis/current-state/testing-analysis.md`, `docs/analysis/current-state/remediation-plan.md`, `docs/technical/api/endpoint-reference.md`, `docs/technical/api/openapi.yaml` | Convierte `MASTER` en plan interno CLARI, alinea `FREE` al mismo catalogo modular base, elimina `Enterprise legacy` y bloquea el acceso owner a `MASTER` salvo para `PlatformOperator` activo. |
| HU-2026-04-03-02 | Estandarización de respuestas de error para model binding y JSON inválido | Core API / Platform Backoffice API / Error Handling | Implementada | 2026-04-03 | `docs/analysis/changes/HU-2026-04-03-02.md` | `docs/technical/api/endpoint-reference.md`, `docs/technical/api/openapi.yaml` | Unifica los `400 common.validation` automáticos con el mismo envelope `ProblemDetails + code + traceId + errors`, elimina prefijos internos en keys y sanea mensajes de conversión JSON. |
| HU-2026-04-03-01 | Catálogos internos globales para requisitos de Job Profiles | Core API / Job Profiles / Internal Catalogs | Implementada | 2026-04-03 | `docs/analysis/changes/HU-2026-04-03-01.md` | `docs/business/current-system-business-flows.md`, `docs/analysis/current-state/architecture-analysis.md`, `docs/analysis/current-state/security-analysis.md`, `docs/analysis/current-state/performance-analysis.md`, `docs/analysis/current-state/testing-analysis.md`, `docs/technical/api/endpoint-reference.md`, `docs/technical/api/openapi.yaml` | Agrega un catálogo interno global no tenant-scoped con búsqueda por similitud, rechazo de casi duplicados y autopoblado desde `JobProfiles` para `Education`, `Knowledge` y `Certification`. |
| HU-BILL-011 | Suscripciones owner, modulos efectivos y marketplace sin pagos | Core API / Platform Billing / Commercial Gating | Implementada | 2026-04-02 | `docs/analysis/changes/HU-BILL-011.md` | `docs/technical/api/endpoint-reference.md`, `docs/technical/api/openapi.yaml` | Agrega superficie owner para cambio inmediato de plan y add-ons, entitlements editables por plan/add-on, gating comercial efectivo y downgrade automatico a `FREE` con desactivacion de add-ons. |
| HU-BILL-010 | Reactivar una suscripcion suspendida | Platform / Billing / Company Subscriptions | Implementada | 2026-04-02 | `docs/analysis/changes/HU-BILL-010.md` | `docs/business/current-system-business-flows.md`, `docs/analysis/current-state/security-analysis.md`, `docs/analysis/current-state/performance-analysis.md`, `docs/analysis/current-state/testing-analysis.md`, `docs/technical/api/endpoint-reference.md`, `docs/technical/api/openapi.yaml` | Agrega preview y reactivacion programada `Suspended -> Active`, solicitudes pendientes auditables, `pendingStatusChange` en overview y procesamiento batch previo a plan-changes y addon-changes. |
| HU-BILL-009 | Activar o desactivar add-ons en una empresa | Platform / Billing / Company Subscription Add-ons | Implementada | 2026-04-02 | `docs/analysis/changes/HU-BILL-009.md` | `docs/business/current-system-business-flows.md`, `docs/technical/api/endpoint-reference.md`, `docs/technical/api/openapi.yaml` | Agrega estado e historial comercial de add-ons por empresa, preview/cambio/cancelacion automatizada y deja alineados los flujos de `plan-changes` ya vigentes en el mismo controlador. |
