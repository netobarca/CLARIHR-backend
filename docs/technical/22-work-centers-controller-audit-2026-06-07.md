...
**WC-A cerrado** como parte de la alineación familia-wide del race dup-code identificado en WorkCenterTypes (WCT-A / doc 21). **WC-B y WC-C NO se tocaron** (no son "el mismo problema de WCT-A"; siguen abiertos como en el audit).
...
| **WC-B** | ⏸️ Abierto | 2 errores de asignación inline fuera del catálogo (localizados); fuera de scope de esta pasada. |
| **WC-C** | ⏸️ Abierto | Cobertura unit ligera de handlers; fuera de scope. |

**Contexto familia-wide:** esta sesión cerró el race dup-code→500 en **toda** la familia Locations que aún lo tenía — `WorkCenterTypes` (WCT-A, doc 21, commit `037533d`), `LocationGroups` y `WorkCenters` (WC-A) — con el patrón CostCenters-R2. `LocationLevels` ya estaba cubierto (LocationHierarchy H-002, `LevelOrder`). Drift-proof por `LocationConstraintViolationsTests` (cada helper acepta su índice y rechaza los hermanos). build **0/0** · unit **1735/1735** · **sin migración · sin resx · sin cambio de contrato** (el `409` ya lo declaraba el pre-check `CodeExistsAsync`).
