...
**Los 3 hallazgos cerrados (WC-A, WC-B, WC-C).** WC-A se cerró antes con la alineación familia-wide (WCT-A / doc 21); WC-B y WC-C se cierran en esta pasada.

| ID | Estado | Remediación |
|---|---|---|
| **WC-A** | ✅ Cerrado | `Create`/`Update`/`Patch` capturan `UniqueConstraintViolationException` (catch tipado antes del genérico → rollback + `409 WORK_CENTER_CODE_CONFLICT`), con el nombre de índice **single-sourced** en `LocationValidationRules.WorkCenterCodeUniqueConstraintName` (== EF `HasDatabaseName`) + helper `LocationConstraintViolations.IsWorkCenterCodeConflict`. `LocationErrors.WorkCenterCodeConflict` ya existía → **sin resx**. |
| **WC-B** | ✅ Cerrado | Los 2 errores inline de `WorkCenterRules.ValidateAssignmentAsync` movidos al catálogo: `LocationErrors.WorkCenterAddressRequired` / `.WorkCenterGeoRequired` (junto a `InvalidCoordinates`). Códigos/mensajes **idénticos** → resx existentes siguen aplicando (sin cambios); `LocationRulesTests` (asierta por código) sigue verde. |
| **WC-C** | ✅ Cerrado | +2 unit de handler en `LocationRulesTests` (reusando los fakes de full-interface ya presentes): `CreateWorkCenter…_WhenCodeAlreadyExists→409` (pre-check) y `…_WhenConcurrentInsertTripsUniqueIndex→409` (valida el catch WC-A end-to-end vía `ThrowingUnitOfWork`). El otro caso recomendado (reassign/level-not-allowed) **ya estaba unit-cubierto** por `WorkCenterRules_ValidateAssignmentAsync_WhenLevelDoesNotAllowWorkCenters` → no se duplica. |

**Contexto familia-wide:** esta sesión cerró el race dup-code→500 en **toda** la familia Locations que aún lo tenía — `WorkCenterTypes` (WCT-A, doc 21, commit `037533d`), `LocationGroups` y `WorkCenters` (WC-A) — con el patrón CostCenters-R2. `LocationLevels` ya estaba cubierto (LocationHierarchy H-002, `LevelOrder`). Drift-proof por `LocationConstraintViolationsTests` (cada helper acepta su índice y rechaza los hermanos). **WC-B/WC-C de esta pasada:** build **0/0** · unit **1737/1737** · **sin migración · sin resx · sin cambio de contrato** (WC-B sólo reubica los 2 Error al catálogo con códigos idénticos; WC-C añade tests).
