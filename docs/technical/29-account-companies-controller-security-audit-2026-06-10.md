...
**Verificación:** `build 0/0 · unit 1789/1789 (+6: 2 pagination guardrail + 3 rate-limit governance + 1 accept-invitation-archivada capa-2) · integración AccountCompany 44/44 (41 base + 3 Access: access-context owner/intruso + role-builder owner) · Auth flows 28/29` (el 1 = `Register_ShouldReturnCreatedAndTokens` topó el limiter `auth-register` 5/min al concentrar 29 auth-tests con IP de test compartida; **pasa aislado** — artefacto del filtro, no regresión). **Gotcha .NET 10 cazado por la integración:** `CompanyStatus[].Contains(x)` en LINQ-to-SQL resuelve a `MemoryExtensions.Contains(ReadOnlySpan)` → EF no traduce → 500; fix = materializar a `List` (CountOwned set-based). AC-2 `IgnoreQueryFilters` en subquery de proyección **confirmado funcional** (Create construye su 201 vía `FindOwnedByUserAsync`).

**Cobertura de AC-1 por capa:** capa 2 (accept-invitation archivada→409) tiene unit test; capa 1 (login/refresh sin `tid`) drift-proofed por el método **abstracto** (fuerza implementación) + query `Status==Active` + login/refresh normales verdes — sin test e2e dedicado (flujo register→create→switch→archive-in-DB de alto costo); capa 3 (revoke al archivar) implementada, sin test e2e dedicado. **Mejora opcional**: integ e2e de capa 1 (archive primaria → login sin tid) y capa 3 (archive → tokens de miembros revocados).
