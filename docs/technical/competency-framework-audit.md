...
**Progreso:** 6/11 resueltos — F1, F4 (PR-A) · F2 (PR-B) · F3, F6, F7 (PR-C), 2026-06-05. Pendientes: 5.
...
| **F9** | ARCH | P3 (low) | ⬜ | **Inconsistencia de código de error**: `CompetencyConductInUse` usa `"RESOURCE_IN_USE"` mientras OPL usa `"OCCUPATIONAL_PYRAMID_LEVEL_IN_USE"`; el cliente no distingue por código. | `CompetencyFrameworkCommon.cs:64-67` | Renombrar a `COMPETENCY_CONDUCT_IN_USE` (revisar consumo previo en el FE antes de romper). |
| **F10** | SEC | P3 (low) | ⬜ | **Defense-in-depth de tenant**: los getters por public-id confían en el filtro global EF (verificado correcto). Sin `.Where(TenantId==)` explícito → frágil ante un futuro `.IgnoreQueryFilters()`. **No es vuln hoy.** | `CompetencyFrameworkRepository.cs:114,276,378` | Opcional: `.Where(e => e.TenantId == tenantId)` explícito en los getters. |
...
| **PR-D** | F5 + F8 + F9 | Cleanup: split god-file, dedupe `ApplyAllowedActions`, código de error. |
| Opcionales | F10, F11 | Endurecimiento; sin bug vivo. |
