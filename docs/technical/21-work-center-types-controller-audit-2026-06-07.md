...
**Sistémico — AHORA CERRADO familia-wide (2026-06-08):** tras este doc, el mismo race dup-code→500 se remedió en **toda** la familia Locations que lo tenía: `LocationGroups` y `WorkCenters` (doc 22 WC-A) recibieron el catch `UniqueConstraintViolationException`→409 idéntico (helpers `LocationConstraintViolations.IsGroupCodeConflict`/`IsWorkCenterCodeConflict` + nombres single-sourced) en la misma sesión. `LocationLevels` ya estaba cubierto desde antes (LocationHierarchy H-002, `LevelOrder`). Drift-proof añadido: `LocationConstraintViolationsTests`. La familia Locations queda alineada (4/4 catálogos con dup-key → 409).
