...
| Platform bank catalogs | `BankCatalogsController` | `/api/platform/bank-catalogs*` | administrar el catalogo global de bancos por pais para consumo del core RH; canonico con `If-Match`/`ETag` + PATCH RFC-6902 (`/code`,`/name`,`/alias`,`/swiftCode`,`/routingCode`,`/sortOrder`; pais inmutable) |
| Platform document type catalogs | `DocumentTypeCatalogsController` | `/api/platform/document-type-catalogs*` | administrar el catalogo global de tipos de documento; canonico con `If-Match`/`ETag` + PATCH RFC-6902 (piloto Ola 3) |
...
- **(Alineación canónica — PR2)** `competency-conducts` es canónico (concurrency-only, sin PATCH): `update`/`activate`/`inactivate` y el `PUT /behaviors` (reemplazo de la colección) exigen `If-Match` (ausente→`400`, stale→`409`); token rotado en body+`ETag`; `POST`→`201`+`ETag`. La matriz aún usa token-en-body (PR3 pendiente).
