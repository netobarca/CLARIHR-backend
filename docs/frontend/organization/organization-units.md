...
`OrgUnitResponse`: `publicId`, `code`, `name`, `orgUnitType{publicId,code,name}`,
`functionalArea{…}|null`, `parent{…}|null`, `sortOrder`, `description`, `costCenterCode`,
`managerEmployeePublicId`, `isActive`, `concurrencyToken`, `createdAtUtc`, `modifiedAtUtc`.
