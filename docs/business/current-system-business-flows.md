...
3. Antes de confirmar la transicion, RRHH puede consultar `GET /api/v1/personnel-files/{id}/finalize/preview` para validar readiness y recibir issues bloqueantes por campo, incluyendo `section`, `fieldKey` y `navigationKey` para que frontend redirija al formulario correspondiente.
